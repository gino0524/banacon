...
## 1. 개요 (Overview)
오래 알고 지낸 친구 15명이 서로의 일정을 한곳에서 공유하는 모바일 우선 웹앱.
...
- 외부 캘린더(구글 캘린더) 연동.
- 그룹/방 다중화 — **단일 그룹(친구 15명) 전제**.
- 실시간 푸시 동기화 — **폴링으로 대체**.

## 3. 사용자 (Users)
- 사전 등록된 **고정 15명**. 회원가입 없음. 사용자 row는 앱에서 삭제하지 않는다.
...
- **참석 기본값**: 일정 생성 시 attendance row를 미리 만들지 **않는다**.
  ...
  - `no_response`는 DB 상태값이 아니라 **화면에서만** 계산해 표시한다(전체 15명 −
...
- 알림 종류 (MVP):
  - `new_event`: 새 일정 등록 시 생성자 제외 14명에게.
...
| 테이블 | 컬럼 | 비고 |
|---|---|---|
| **users** | `name varchar(40) UNIQUE NOT NULL`, `avatar_color text` | 사전 시드 15명, 앱에서 삭제 안 함 |
...
| `/login` | 공유 초대코드 입력 + 이름 그리드(15명) 선택 → 세션 쿠키 발급 |
...
- **new_event 알림**: `createEvent` 액션이 본인 제외 14명에게 notifications 일괄 insert.
...
- **성능 규모**: 동시 사용자 ~15명, 데이터 소량 — 폴링 부하 무시 가능.
...
5. 아무도 응답 안 한 일정 → 명단에 **미응답 15명**으로 표시. 일부 응답 시 미응답
...
- `.env.example`, `.env.local`(SESSION_SECRET 생성됨, APP_INVITE_CODE=`banacon2026`,
  CRON_SECRET 임시값, **DATABASE_URL은 비어 있음**).
- `package.json` 스크립트: `db:push / db:generate / db:studio / db:seed`.
- `next.config.ts`에 `turbopack.root` 고정.
- 검증 완료: `tsc --noEmit` OK, `npm run build` OK.

**P1 시작 전 사용자에게 받아야 할 것 (차단 요소):**
1. **`DATABASE_URL`** (Neon connection string) → `.env.local`에 채워야 `db:push`/시드 검증 가능.
   값이 없으면 스키마/시드 코드까지만 작성하고 검증은 대기.
2. **친구 15명 실제 이름** → 시드용. 없으면 `친구1~친구15` placeholder로 진행(나중에 교체).

**기타 상태:** git 미설치 → 단계별 커밋 보류 중(`winget install --id Git.Git -e` 후 가능). dev/build엔 영향 없음.
...
- **목표**: 5장 데이터 모델을 Drizzle로 정의하고 마이그레이션 + 친구 15명 시드.
- **산출물**:
  ...
  - 시드 스크립트(`drizzle/seed.ts` 또는 `package.json` script) — 친구 15명 이름·avatar_color.
    *(실제 이름 15개는 사용자에게 받거나 placeholder로 두고 사용자가 교체하도록 명시.)*
- **완료 조건(DoD)**: `drizzle-kit push` 성공, DB에 5개 테이블 + users 15행 확인.
...
- **산출물**:
  ...
  - `src/app/login/page.tsx` — 초대코드 입력 + 15명 이름 그리드.
...
- **산출물**:
  ...
  - 명단: 가는중/미정/불참 + **미응답 = 15 − 응답자**(화면 계산, F1).
...
- **산출물**:
  - `createEvent`에서 본인 제외 14명에게 `notifications` 일괄 insert(type=new_event).
